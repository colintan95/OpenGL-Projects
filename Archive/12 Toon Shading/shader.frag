...
void main() {
    ...
    //Normal Diffues Lighting
    //float intensity = max(dot(norm, light), 0);
    
    //Toon Shading
    float intensity = max(floor(dot(norm, light) * toonLevels) / toonLevels, 0);
    ...
    vec3 rgb = uAmbient * baseColor.rgb + shadow * (intensity * uDiffuse * attenuation);
