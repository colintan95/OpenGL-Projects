...
void glInit() {
    ...
    gRenderVS = CreateShader(GL_VERTEX_SHADER, "wave.vert");
    gRenderFS = CreateShader(GL_FRAGMENT_SHADER, "wave.frag");
    ...
    gWavenumberLoc = glGetUniformLocation(gRenderProg, "uWavenumber");
    float wavenumber = 1.f;
    glUniform1f(gWavenumberLoc, wavenumber);
    
    gVelocityLoc = glGetUniformLocation(gRenderProg, "uVelocity");
    float velocity = 5.f;
    glUniform1f(gVelocityLoc, velocity);
    
    gAmplitudeLoc = glGetUniformLocation(gRenderProg, "uAmplitude");
    float amplitude = 1.f;
    glUniform1f(gAmplitudeLoc, amplitude);
    
    gTimeLoc = glGetUniformLocation(gRenderProg, "uTime");
    ...
    Model sheet = ModelFactory::BuildSheet(40.f, 40.f, 100, 100);
    
    GLuint sheetBuffers[3];
    glGenBuffers(3, &sheetBuffers[0]);
    
    glBindBuffer(GL_ARRAY_BUFFER, sheetBuffers[0]);
    glBufferData(GL_ARRAY_BUFFER, sheet.m_Info.vertCount * 4 * sizeof(float), &sheet.m_Info.v[0], GL_STATIC_DRAW);
    
    glBindBuffer(GL_ARRAY_BUFFER, sheetBuffers[1]);
    glBufferData(GL_ARRAY_BUFFER, sheet.m_Info.vertCount * 2 * sizeof(float), &sheet.m_Info.vt[0], GL_STATIC_DRAW);
    
    glBindBuffer(GL_ARRAY_BUFFER, sheetBuffers[2]);
    glBufferData(GL_ARRAY_BUFFER, sheet.m_Info.vertCount * 3 * sizeof(float), &sheet.m_Info.vn[0], GL_STATIC_DRAW);
    
    gSheetObj.glPosBuf = sheetBuffers[0];
    gSheetObj.glTexCoordBuf = sheetBuffers[1];
    gSheetObj.glNormBuf = sheetBuffers[2];
    
    gSheetObj.vertCount = sheet.m_Info.vertCount;
    
    gSheetObj.ambientColor = {0.5f, 0.3f, 0.3f};
    gSheetObj.diffuseColor = {0.8f, 0.f, 0.f};
    gSheetObj.specularColor = {1.f, 1.f, 1.f};
    
    gSheetObj.shininess = 20.f;
    gSheetObj.specStrength = 0.f;
    
    gSheetObj.isTextured = true;
    gSheetObj.objColor = {0.1f, 0.f, 0.f, 1.f};
    
    gSheetObj.glTexture = TeapotTexture;
    gSheetObj.glTexUnit = 1;
    
    gSceneGraph.push_back(gSheetObj);
}
...
void glLoop() {
    ...
    float time = std::chrono::duration_cast<std::chrono::microseconds>(currentTime - gStartTime).count() / 1000000.f;
    
    glUniform1f(gTimeLoc, time);
