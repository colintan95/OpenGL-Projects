...
void glInit() {
    ...
    glm::vec4 lightPos = {0.f, 8.f, 30.f, 1.f};
    glUniform4fv(gLightPosLoc, 1, glm::value_ptr(lightPos));
    
    gLightStrengthLoc = glGetUniformLocation(gRenderProg, "uLightStrength");
    float strength = 5.f;
    glUniform1f(gLightStrengthLoc, strength);
    ...
    glm::vec3 coneDir = {-0.f, 0.f, -1.f};
    glUniform3fv(gConeDirectionLoc, 1, glm::value_ptr(coneDir));
    
    gConeLimitLoc = glGetUniformLocation(gRenderProg, "uConeLimit");
    float coneLimit = 0.98f;
    glUniform1f(gConeLimitLoc, coneLimit);
