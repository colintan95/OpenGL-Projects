...
GLuint gFilterFbo;
GLuint gFilterDepthRbo;
GLuint gFilterTexture;
...
void glInit() {
    ...
    gFilterVS = CreateShader(GL_VERTEX_SHADER, "edge_detection.vert");
    gFilterFS = CreateShader(GL_FRAGMENT_SHADER, "edge_detection.frag");
    ...
    gFilterThresholdLoc = glGetUniformLocation(gFilterProg, "uEdgeThreshold");
    glUniform1f(gFilterThresholdLoc, 0.1f);
    ...
    glGenFramebuffers(1, &gFilterFbo);
    glBindFramebuffer(GL_FRAMEBUFFER, gFilterFbo);
    
    glGenTextures(1, &gFilterTexture);
    glActiveTexture(GL_TEXTURE3);
    glBindTexture(GL_TEXTURE_2D, gFilterTexture);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, gFilterTexture, 0);
    
    glGenRenderbuffers(1, &gFilterDepthRbo);
    glBindRenderbuffer(GL_RENDERBUFFER, gFilterDepthRbo);
    
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, gScreenWidth * 2, gScreenHeight * 2);
    
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, gFilterDepthRbo);
